...
server.get('/todos', function(req, res) {
  ...
  fs.readFile('./todos.json', 'utf8' , (err, todosLeidos) => {
    if (err) {
      return res.sendStatus(400).send('Bad request')
    }else{
      return res.json(JSON.parse(todosLeidos));
    }
...
server.put('/todos/:id', function(req, res) {
  ...
  const indexTodo = todos.findIndex(el => el.id == idUrl);
...
//Patch comprobando el title
// server.patch('/todos/:id', function(req, res) {
//   const idUrl = req.params.id;
//   const todo = todos.find(el => el.id == idUrl);
//   if(todo){
//     if(req.body.title){
...
server.delete('/todos/:id', function(req, res) {
  const idUrl = req.params.id;
  let todosRead = JSON.parse(fs.readFileSync('./todos.json'));
  const indexTodo = todosRead.findIndex(el => el.id == idUrl);
  const todo = todosRead[indexTodo];
  if(indexTodo !== -1){
    todosRead.splice(indexTodo, 1);
    fs.writeFileSync('./todos.json', JSON.stringify(todosRead));
    res.json(todo);
  }else{
    res.sendStatus(400).send('Bad request')
  }
